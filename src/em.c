...
em_config_t em_config_default(void) {
    return (em_config_t){
        .max_iter = PARAM_EM_MAX_ITER,
        .conv_threshold = PARAM_EM_CONV_THRESHOLD,
        .n_restarts = PARAM_EM_N_RESTARTS,
        .alpha = PARAM_EM_ALPHA,
        .d_mu = PARAM_EM_D_MU,
        .d_sigma = PARAM_EM_D_SIGMA,
        .b_mu = PARAM_EM_B_MU,
        .b_sigma = PARAM_EM_B_SIGMA,
        .estimate_degradation = 0,
        .min_containment = PARAM_EM_MIN_CONTAINMENT,
        .seed = PARAM_EM_SEED,
        .mito_copy_numbers = NULL,
        .prune_threshold = PARAM_EM_PRUNE_THRESHOLD,
        .use_advanced_ci = 1,   /* default: Louis (1982) observed Fisher CI */
        .use_brent_lambda = 0,
        .use_full_lrt = 1,      /* default: full nested-model re-fit LRT */
    };
}
...
em_result_t *em_fit(const em_read_t *reads, int n_reads,
    ...
    /* BIC = -2*LL + k*ln(n)
     * In single-marker mode: d is fixed from mito CN priors, b = 1.0.
     * Only w is free: k = S-1.
     * In multi-marker mode: k = S-1 (w) + S (d) + S*M (b) [+ 1 (lambda)] */
    int n_params;
    if (single_marker) {
        n_params = n_species - 1;
    } else {
        n_params = (n_species - 1) + n_species + n_species * n_markers;
    }
...
void em_lrt_full(em_result_t *result,
    ...
    for (int s_test = 0; s_test < S; s_test++) {
        ...
        for (int r = 0; r < n_reads; r++) {
            ...
            if (nc_new == 0) { n_reads_dropped++; continue; }
        ...
        /* If reads were dropped when removing this species, the reduced dataset
         * differs from the full dataset. A full nested LRT is invalid (datasets
         * must be identical). Fall back to profile LRT: zero weight, keep
         * nuisance parameters fixed, re-evaluate LL on full dataset. */
        double lrt;
        if (n_reads_dropped > 0) {
            double *w_null = (double *)hs_malloc((size_t)S * sizeof(double));
            double sum_null = 0.0;
            for (int i = 0; i < S; i++) {
                w_null[i] = (i == s_test) ? 0.0 : result->w[i];
                sum_null += w_null[i];
            }
            if (sum_null > 0) {
                for (int i = 0; i < S; i++) w_null[i] /= sum_null;
                ...
                for (int r = 0; r < n_reads; r++) {
                    ...
                    if (has_valid) {
                        ...
                        for (int j = 0; j < nc; j++) {
                            int sp = reads[r].species_indices[j];
                            if (w_null[sp] > 0) s += exp(log_g[j] - max_log);
                        }
                        ll_null += max_log + log(s);
                    } else {
                        ll_null += -100.0;
                    }
                    free(log_g);
                }
                lrt = 2.0 * (result->log_likelihood - ll_null);
            } else {
                lrt = 0.0;
            }
            free(w_null);
            ...
            reduced_cfg.n_restarts = 1;
            reduced_cfg.max_iter = 50;
            reduced_cfg.use_advanced_ci = 0;
            reduced_cfg.use_brent_lambda = 0;
            reduced_cfg.use_full_lrt = 0;
            reduced_cfg.prune_threshold = 0.0;
            reduced_cfg.mito_copy_numbers = NULL;
            ...
            lrt = 2.0 * (result->log_likelihood - ll_reduced);
        }
        if (lrt < 0) lrt = 0;
